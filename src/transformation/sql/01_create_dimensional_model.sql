...
SELECT
    cnes,
    MAX(nome_estabelecimento) AS nome_estabelecimento,
    MAX(co_ibge) AS co_ibge,
    MAX(co_tipo_unidade) AS co_tipo_unidade,
    SUM(leitos_existentes) AS leitos_existentes,
    SUM(leitos_sus) AS leitos_sus,
    SUM(uti_total_exist) AS uti_total,
    MAX(tp_gestao) AS tp_gestao
FROM raw_leitos
GROUP BY cnes;
